MONITOR_JS = r'''
var charts={};
var CHART_FONT='system-ui,-apple-system,Segoe UI,Roboto,sans-serif';
var CHART_MONO='ui-monospace,SFMono-Regular,Menlo,Consolas,monospace';
var chartColors={
...
// One place for the shared Chart.js look; per-chart tweaks are merged on top.
function baseOpts(o){
  o=o||{};
  var ct=chartTheme();
  var opts={
    ...
    plugins:{
      legend:{display:true,labels:{color:ct.legend,font:{family:CHART_FONT,size:10},boxWidth:8,boxHeight:8,padding:12,usePointStyle:true}},
      tooltip:{backgroundColor:ct.tipBg,borderColor:ct.tipBorder,borderWidth:1,titleColor:ct.tipTitle,bodyColor:ct.tipBody,
        titleFont:{family:CHART_FONT,size:12,weight:600},bodyFont:{family:CHART_MONO,size:11},
        padding:10,cornerRadius:8,displayColors:true,boxWidth:8,boxHeight:8,boxPadding:4}},
    scales:{
      x:{display:true,ticks:{color:ct.tick,font:{family:CHART_MONO,size:9},maxTicksLimit:o.xTicks||8,maxRotation:0},grid:{display:false},border:{display:false}},
      y:{display:true,ticks:{color:ct.tick,font:{family:CHART_MONO,size:9},callback:o.yCallback||undefined,maxTicksLimit:5},grid:{color:ct.grid},border:{display:false}}}};
  if(o.tooltipLabel)opts.plugins.tooltip.callbacks={label:o.tooltipLabel};
  if(o.yTitle){opts.scales.y.position='left';opts.scales.y.title={display:true,text:o.yTitle,color:ct.tick,font:{size:9}}}
  if(o.y1Title){opts.scales.y1={display:true,position:'right',title:{display:true,text:o.y1Title,color:ct.tick,font:{size:9}},
    ticks:{color:ct.tick,font:{family:CHART_MONO,size:9},maxTicksLimit:5},grid:{display:false},border:{display:false},min:0}}
  return opts}
...
function clearChart(key){if(charts[key]){try{charts[key].destroy()}catch(e){}delete charts[key]}}

function drawAllCharts(){
  if(!window.Chart)return;
  if(!S.history||!S.history.snapshots||!S.history.snapshots.length){clearChart('sess');clearChart('res');return}
  var span=S.monPeriod;
  var d=S.history.snapshots;
  var labels=d.map(function(x){return ts2t(x.ts,span)});

  var c1=document.getElementById('ch-sessions');
  if(c1){updateChart('sess',c1,{type:'line',data:{labels:labels,datasets:[mkDataset(t('active_sessions'),d.map(function(x){return x.sessions}),chartColors.sessions)]},options:baseOpts()})}

  var c3=document.getElementById('ch-resources');
  if(c3){
    var memLabel=t('memory')+' MB';
    ...
    updateChart('res',c3,{type:'line',data:{labels:labels,datasets:[
      {label:memLabel,data:d.map(function(x){return x.mem/1048576}),borderColor:chartColors.memory.border,backgroundColor:chartColors.memory.bg,fill:true,tension:.35,pointRadius:0,borderWidth:1.5,yAxisID:'y'},
    ...
    ]},options:baseOpts({yTitle:memLabel,y1Title:'CPU %'})})}
}
...
function drawTrafficChart(){
  if(!window.Chart)return;
  var c=document.getElementById('ch-traffic');
  if(!S.traffic||!S.traffic.hourly||!S.traffic.hourly.length){clearChart('traffic');return}
  var d=S.traffic.hourly;
  var interval=3600;
...
function drawConnChart(){
  if(!window.Chart)return;
  var c=document.getElementById('ch-conns');
  if(!S.connTimeline||!S.connTimeline.timeline||!S.connTimeline.timeline.length){clearChart('conns');return}
  var d=S.connTimeline.timeline;
  if(c){
    ...
    updateChart('conns',c,{type:'bar',data:{
      labels:d.map(function(x){return ts2t(x.ts,S.monPeriod)}),
      datasets:[
      ...
      ]},options:baseOpts({yTitle:t('connections'),y1Title:t('unique_ips_label')})})
  }
}
    ...
    h('div',{className:'tbl-wrap'},
      h('table',{className:'tbl'},
        h('thead',null,h('tr',null,
          h('th',null,t('username')),
          h('th',null,t('sessions')),
          h('th',{style:{textAlign:'right'}},t('download')),
          h('th',{style:{textAlign:'right'}},t('upload')),
          h('th',null,t('ip_address')),
          h('th',null,t('last_seen')))),
        h('tbody',null,rows.map(function(u){
          var online=(u.sessions||0)>0;
          var g=u.geo||{};
          return h('tr',null,
...
function renderMonitor(){
  ...
  var connRows=S.conns&&S.conns.connections?S.conns.connections:[];
  ...
  return h('div',{className:'tab-content'},
    h('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'14px'}},
      ...
      periodSelector(S.monPeriod,periods,function(v){S.monPeriod=v;S.connPeriod=v;Promise.all([_loadHistory(v),_loadTraffic(v),_loadConnTimeline(),_loadConns(v),_loadOnline(),_loadPerUser()]).then(function(){R(drawMonitorCharts)})})),
    ...
    h('div',{className:'card'},
      ...
      S.online&&S.online.online_users&&S.online.online_users.length?
        h('div',{className:'tbl-wrap',style:{maxHeight:'200px',overflow:'auto'}},
          h('table',{className:'tbl'},
      ...
      connRows.length?h('div',null,
        h('div',{className:'tbl-wrap'},
          h('table',{className:'tbl',style:{tableLayout:'fixed',width:'100%'}},
            ...
            // rows only — the pager lives outside the table so the grid stays valid
            h('tbody',null,pageSlice('connlog',connRows,25).map(function(c){return h('tr',null,
    ...
    h('div',{style:{display:'flex',justifyContent:'space-between',fontSize:'10px',color:'var(--tx3)',fontFamily:'var(--m)',marginTop:'8px',padding:'0 4px'}},
      ...
      S.dbSize?h('span',null,t('db_size')+': '+S.dbSize.db_mb+' MB | '+t('log_size')+': '+S.dbSize.log_mb+' MB'):null)
  );
}
